---
title: "Bottleneck exploratory data analysis"
author: "Zena Lapp"
date: "9/1/2021"
output: 
  html_document:
...
pcr_results <- bind_rows(human_dat,
  ...
  left_join(ama_haplotype_summary) %>% 
...
pcr_plot <- pcr_results %>% 
  mutate(pcr_seq = ifelse(grepl('Negative', pcr_seq), 'Uninfected', gsub('Positive ', 'Infected (', pcr_seq)),
         ...
         pcr_seq = factor(pcr_seq, levels = c('Uninfected','Infected (neither)', 'Infected (*ama1* only)',
                                              'Infected (*csp* only)', 'Infected (*ama1* and *csp*)')),
...
ah_pairs <- bind_rows(ah_csp, ah_ama) %>% filter(!is.na(Abdomen) & !is.na(Head)) 
...
ah_hap_cts <- sapply(1:nrow(ah_pairs), function(x){
  ...
  c(sample_name_dbs = sample_name_dbs, marker = marker, 
    n_abd = length(a), n_head = length(h), 
...
all_ah_hap_props <- sapply(ah_pairs$Abdomen, function(a){
  a <- strsplit(a, ',')[[1]]
  jaccards <- sapply(ah_pairs$Head, function(h){
    h <- strsplit(h, ',')[[1]]
...
mosq_comp_compare <- ah_hap_cts %>% 
  ...
  geom_col() +
  labs(y = 'Marker', x = 'Mean proportion of haplotypes in\nabdomens and heads from the same mosquito', 
       fill = 'Haplotype\npresent in:') +
...
f1 <- plot_grid(pcr_compare, mosq_comp_compare, mosq_pop_compare, 
                labels = 'AUTO',  nrow = 1, rel_widths = c(0.75, 1, 1)) 

ggsave(plot = f1, filename = 'figures/Fig1.png', width = 13, height = 3.5)
```
...
fs2 <- plot_grid(mosq_hap_union, jaccard_plot, jaccard_compare, nrow = 1, rel_widths = c(1.2, 0.9, 0.8), labels = 'AUTO')

ggsave(plot = fs2, filename = 'figures/FigS2.png', width = 15, height = 4)
```
...
hap_abundance_plot <- compartments_present %>% 
  ...
  scale_fill_manual(values = c('Human\nonly' = host_colors['Human'], Both = '#999999', 'Mosquito\nonly' = host_colors['Mosquito']) ) +
...
plot_rmst_tr <- function(seqs, marker){
  ...
  rmst_plot <- ggtree(mst_phylo) + 
    ...
    scale_color_manual(values = c('Human only' = host_colors['Human'], Both = '#999999', 'Mosquito only' = host_colors['Mosquito'])) +
...
### Figure 2 & S6: The mosquito _P. falciparum_ population is more diverse than the human P. falciparum population.
...
ama_evenness <- Evenness(ama_inc_dat, q = seq(0, 2, by = 0.25), datatype = 'incidence_freq', 
                         method = "Estimated", nboot = 100, conf = 0.95, E.class = 3)
ama_Cmax = ama_evenness$Coverage
csp_evenness <- Evenness(csp_inc_dat, q = seq(0, 2, by = 0.25), datatype = 'incidence_freq', 
                         method = "Estimated", nboot = 100, conf = 0.95, E.class = 3)
...
diversity_plot_supp <- bind_rows(
  ...
  bind_rows(ama_asy3D %>% mutate(marker = '*ama1*'), 
    ...
    mutate(samp_set = 'True diversity (asymptotic)') %>% 
    filter(Order.q >= 1)) %>% 
    ...
    labs(x = 'Order (q)', y = 'Population-level<br>haplotype diversity (^q D)', fill = 'Host', col = 'Host', shape = 'Sample set')
...
```{r}
fig_div_supp <- plot_grid(raref_plot, diversity_plot_supp, labels = c('A', 'B'), nrow = 2)
ggsave(plot = fig_div_supp, filename = 'figures/FigS6.png', width = 12, height = 7)
```
...
ggsave(plot = plot_grid(read_dominance_plot_comb, read_dominance_plot_raw, rel_widths = c(0.6, 1), labels = 'AUTO'), 
       filename = 'figures/FigS7.png', width = 8, height = 4)
```
